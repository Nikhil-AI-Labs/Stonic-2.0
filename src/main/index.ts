...
import migrateLegacyData from './logic/legacy-migration'
import registerIpcHandlers from './logic/stonic-memory-save'
...
import registerStonicCoder from './services/stonic-coder'
...
// `stonic://` is the primary deep-link scheme. `iris://` stays registered so OAuth
// callbacks issued by the existing backend continue to reach the app.
const PROTOCOL_SCHEMES = ['stonic', 'iris']
const isDeepLink = (arg: string): boolean =>
...
const secureConfigPath = join(app.getPath('userData'), 'stonic_secure_vault.json')
...
app.on('second-instance', (event, commandLine) => {
  ...
  if (mainWindow) {
    ...
    mainWindow.focus()
    const url = commandLine.find((arg) => isDeepLink(arg))
    if (url) {
...
app.whenReady().then(() => {
  ...
  app.on('open-url', (event, url) => {
    ...
    if (mainWindow && isDeepLink(url)) {
      mainWindow.webContents.send('oauth-callback', url)
    }
  })

  // Must run before any handler reads persisted state (vault, memory, workflows).
  ...
  registerStonicCoder({ ipcMain, app })
