...
export default function registerIpcHandlers({ ipcMain, app }: { ipcMain: IpcMain; app: App }) {
  ...
  const FILE_PATH = path.join(CHAT_DIR, 'stonic_memory.json')
    ...
    try {
      if (fs.existsSync(FILE_PATH)) {
        ...
        return raw.map((m: any) => ({
          // 'iris' is the pre-rebrand tag — still mapped so old history stays readable.
          role: m.role === 'stonic' || m.role === 'iris' ? 'model' : m.role,
