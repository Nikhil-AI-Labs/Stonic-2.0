...
export default function registerSecurityVault() {
  const legacyFace = store.get('stonic_vault_face') as number[] | undefined
  if (legacyFace && !store.get('stonic_vault_faces')) {
    store.set('stonic_vault_faces', [legacyFace])
    store.delete('stonic_vault_face')
  }

  ipcMain.handle('check-vault-status', () => {
    const hasPin = !!store.get('stonic_vault_hash')
    const faces = store.get('stonic_vault_faces') as number[][] | undefined
    const hasFace = faces && faces.length > 0
    return { hasPin, hasFace, faceCount: faces ? faces.length : 0 }
  })

  ipcMain.handle('get-personality', () => {
    return store.get('stonic_personality') as string | undefined
  })

  ipcMain.handle('set-personality', (_, text: string) => {
    store.set('stonic_personality', text)
    return true
  })
  ...
  ipcMain.handle('setup-vault-pin', async (_, pin: string) => {
    ...
    store.set('stonic_vault_hash', hash)
    return true
  })

  ipcMain.handle('verify-vault-pin', async (_, pin: string) => {
    const hash = store.get('stonic_vault_hash') as string
    if (!hash) return false
    return await bcrypt.compare(pin, hash)
  })

  ipcMain.handle('setup-vault-face', (_, descriptor: number[]) => {
    const faces = (store.get('stonic_vault_faces') as number[][]) || []
    faces.push(descriptor)
    store.set('stonic_vault_faces', faces)
    return true
  })
