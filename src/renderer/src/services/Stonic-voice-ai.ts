...
import { getHistory, retrieveCoreMemory, saveCoreMemory, saveMessage } from './stonic-ai-brain'
...
export class GeminiLiveService {
  ...
  async connect(): Promise<void> {
    if (window.electron?.ipcRenderer) {
      ...
      this.apiKey = secureKeys?.geminiKey || localStorage?.getItem('stonic_custom_api_key') || ''
    } else {
      this.apiKey = localStorage.getItem('stonic_custom_api_key') || ''
    }
    ...
    let cloudUser = {
      name: localStorage.getItem('stonic_user_name') || 'Nikhil',
      email: 'Not linked'
    }
    ...
    const STONIC_SYSTEM_INSTRUCTION = `
# 👁️ STONIC 2.0 — YOUR INTELLIGENT COMPANION (Project JARVIS)
You are **STONIC 2.0**, a high-performance AI agent. You don't just talk; you **execute**.
...
- **Example:** "Stonic, find my code and send it to Nikhil on WhatsApp."
...
`
    ...
    const finalSystemInstruction = STONIC_SYSTEM_INSTRUCTION + contextPrompt
          ...
          tools: [
            {
              functionDeclarations: [
                  ...
                  parameters: {
                    ...
                    properties: {
                      title: {
                        type: 'STRING',
                        description:
                          'A short, descriptive title for the note (e.g., "Project_Stonic_Plan").'
                      },
                      content: {
                        type: 'STRING',
                        description:
                  ...
                  parameters: {
                    ...
                    properties: {
                      source_path: {
                        type: 'STRING',
                        description:
                          'The absolute file path on the PC (e.g., "C:/Users/User/Desktop/document.pdf").'
                      },
                      dest_path: {
                        type: 'STRING',
                        description:
                  ...
                  parameters: {
                    ...
                    properties: {
                      ...
                      custom_text: {
                        type: 'STRING',
                        description:
                          'If rewriting text, generate a highly cinematic, hacker-style headline to inject into the website. (e.g., "STONIC 2.0 HAS TAKEN OVER", or whatever the user requested).'
                      }
                    },
                    required: ['url', 'mode']
                  }
                ...
                },
                {
                  ...
                  description:
                    'ACTION: Spawns the STONIC 2.0 Live Forge and generates a full, highly animated, real-time website using Tailwind CSS and GSAP. Use this when the user asks you to build a landing page, a portfolio, a 3D site, or a complex web interface.',
                  parameters: {
                    type: 'OBJECT',
                    properties: {
                      prompt: {
                  ...
                  parameters: {
                    ...
                    properties: {
                      base_directory: {
                        type: 'STRING',
                        description:
                          'The absolute path of the root folder being sorted (e.g., "C:\\Users\\User\\Downloads").'
                      },
                      files_to_sort: {
                        type: 'ARRAY',
                        items: {
                ...
                },
                {
                  ...
                  description:
                    'Instantly locks the STONIC 2.0 OS system, disconnects the AI, and returns the user to the secure biometric lock screen. Use this strictly when the user says "Lock the system", "Lock down", or "Activate Sentry Mode".',
                  parameters: {
                    type: 'OBJECT',
                    properties: {}
                  }
          ...
          generationConfig: {
            ...
            speechConfig: {
              voiceConfig: {
                prebuiltVoiceConfig: {
                  voiceName:
                    localStorage.getItem('stonic_voice_profile') === 'FEMALE' ? 'Aoede' : 'Puck'
                }
              }
            }
          },
        ...
        if (serverContent) {
          ...
          if (serverContent.turnComplete || serverContent.interrupted) {
            ...
            if (this.aiResponseBuffer.trim()) {
              await saveMessage('stonic', this.aiResponseBuffer.trim())
              this.aiResponseBuffer = ''
            }
          }
        }
...
export const stonicService = new GeminiLiveService()
