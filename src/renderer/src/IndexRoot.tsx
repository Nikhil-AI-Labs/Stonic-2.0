...
import { stonicService } from './services/Stonic-voice-ai'
import { getScreenSourceId } from './hooks/CaptureDesktop'
import STONIC from './UI/STONIC'
...
const IndexRoot = () => {
  ...
  useEffect(() => {
    const watchdog = setInterval(() => {
      if (isSystemActive && !stonicService.isConnected) {
  ...
  const toggleSystem = async () => {
    if (!isSystemActive) {
      try {
        await stonicService.connect()
        setIsSystemActive(true)
        setIsMicMuted(false)
        stonicService.setMute(false)
      } catch (err: any) {
      ...
      stonicService.disconnect()
      setIsSystemActive(false)
      setIsMicMuted(true)
      stonicService.setMute(true)
      stopVision()
    }
  }
  ...
  const startAIProcessing = () => {
    ...
    aiIntervalRef.current = setInterval(() => {
      ...
      if (vid && vid.readyState === 4 && stonicService.socket?.readyState === WebSocket.OPEN) {
        ...
        if (ctx) {
          ...
          stonicService.sendVideoFrame(base64)
        }
      }
    }, 2000)
  }
