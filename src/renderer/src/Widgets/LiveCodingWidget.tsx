...
export default function LiveCodingWidget() {
  ...
  useEffect(() => {
    if (monaco) {
      monaco.editor.defineTheme('stonic-dark', {
        base: 'vs-dark',
        inherit: true,
        rules: [{ token: 'comment', foreground: '10b981', fontStyle: 'italic' }],
        colors: { 'editor.background': '#00000000' }
      })
      monaco.editor.setTheme('stonic-dark')
    }
  }, [monaco])
  ...
  useEffect(() => {
    const handleStartCoding = async (e: any) => {
      ...
      const geminiKey = localStorage.getItem('stonic_custom_api_key') || ''
      ...
      setCode('// Initializing STONIC 2.0 Neural Forge...\n')
