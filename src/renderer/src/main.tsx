...
import { migrateLegacyStorage } from './utils/legacy-migration'

// Carry any pre-rebrand `iris_` entries over before a component reads them.
  ...
  useEffect(() => {
    if (electronAPI) {
      electronAPI.on('oauth-callback', (_event: any, url: string) => {
        try {
          const urlObj = new URL(url.replace(/^(?:stonic|iris):\/\//, 'http://localhost/'))
          const refreshToken = urlObj.searchParams.get('refreshToken')
          const accessToken = urlObj.searchParams.get('accessToken')
          if (refreshToken && accessToken) {
            localStorage.setItem('stonic_cloud_token', refreshToken)
            navigate('/')
          }
        } catch (e) {}
      })
