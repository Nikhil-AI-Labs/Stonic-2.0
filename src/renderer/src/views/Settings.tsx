...
const SettingsView = ({ isSystemActive }: SettingsProps) => {
  ...
  const [voice, setVoice] = useState<'MALE' | 'FEMALE'>(
    (localStorage.getItem('stonic_voice_profile') as 'MALE' | 'FEMALE') || 'MALE'
  )
  const [personality, setPersonality] = useState('')
  const [userName, setUserName] = useState(localStorage.getItem('stonic_user_name') || '')

  const [geminiKey, setGeminiKey] = useState(localStorage.getItem('stonic_custom_api_key') || '')
  const [groqKey, setGroqKey] = useState(localStorage.getItem('stonic_groq_api_key') || '')
  const [hfKey, setHfKey] = useState(localStorage.getItem('stonic_hf_api_key') || '')
  const [tailvyKey, setTailvyKey] = useState(localStorage.getItem('stonic_tailvy_api_key') || '')
  ...
  const handleVoiceChange = (v: 'MALE' | 'FEMALE') => {
    ...
    setVoice(v)
    localStorage.setItem('stonic_voice_profile', v)
  }
  ...
  const saveUserName = () => {
    localStorage.setItem('stonic_user_name', userName)
    alert('User Designation Saved.')
  }

  const saveApiKeys = async () => {
    localStorage.setItem('stonic_custom_api_key', geminiKey)
    localStorage.setItem('stonic_groq_api_key', groqKey)
    localStorage.setItem('stonic_hf_api_key', hfKey)
    localStorage.setItem('stonic_tailvy_api_key', tailvyKey)
              ...
              >
                <div className={`${cardClass} md:col-span-2`}>
                  ...
                  <textarea
                    ...
                    placeholder="Define who STONIC 2.0 is. Example: 'You are a sassy, highly technical assistant...'"
                  ...
                  <div className="flex justify-between items-center">
                    ...
                    {isSystemActive && (
                      <span className="text-[10px] text-red-400 font-mono tracking-widest flex items-center gap-1 bg-red-500/10 px-2 py-1 rounded border border-red-500/20">
                        <RiLock2Line /> LOCKED AS STONIC 2.0 IS CONNECTED
                      </span>
                    )}
                  </div>
                  <div
                ...
                <div className={`${cardClass} gap-6`}>
                  ...
                  <div className="bg-[#050505] border border-white/5 p-4 rounded-xl mt-2 flex items-start gap-3">
                    ...
                    <p className="text-[10px] text-zinc-400 font-mono leading-relaxed">
                      ...
                      local OS. STONIC 2.0 does not transmit these keys to any centralized server. You
