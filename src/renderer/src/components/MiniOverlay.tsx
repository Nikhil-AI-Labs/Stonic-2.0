...
import { stonicService } from '@renderer/services/Stonic-voice-ai'
...
const MiniOverlay = ({
  ...
  useEffect(() => {
    if (isSystemActive && stonicService.analyser) {
      analyzerRef.current = stonicService.analyser
      dataArrayRef.current = new Uint8Array(stonicService.analyser.frequencyBinCount)
      const checkAudio = () => {
